...
public class OrderStatusController {
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Order status found", content = @Content(schema = @Schema(implementation = OrderStatusResponse.class), examples = @ExampleObject(name = "OrderStatus", value = """
                                        {
                                          "id": 1,
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Statuses returned successfully", content = @Content(array = @ArraySchema(schema = @Schema(implementation = OrderStatusResponse.class)), examples = @ExampleObject(name = "OrderStatusList", value = """
                                                                    Allowed transitions:
                                        - SUBMITTED → PAID, CANCELED
                                        - PAID → IN_PROGRESS, REFUNDED
                                        - IN_PROGRESS → READY_FOR_PACKING
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Status names returned successfully", content = @Content(array = @ArraySchema(schema = @Schema(implementation = String.class)), examples = @ExampleObject(name = "StatusValues", value = """
                                        [
                                          "SUBMITTED",
                                          "PAID",
                                          "IN_PROGRESS",

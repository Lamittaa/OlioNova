...
@Tag(name = "Cities", description = "Used to link customers to cities.")
public class CityController {
    ...
    @Operation(summary = "Create a new city", description = "Creates a new city in the city lookup table.", requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "City creation payload", required = true, content = @Content(examples = @ExampleObject(name = "CreateCityRequest", value = """
            {
              "cityName": "Bethlehem"
            }
            """))))
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "City created successfully", content = @Content(schema = @Schema(implementation = CreateCityResponse.class), examples = @ExampleObject(name = "CreatedCity", value = """
                    {
                      "id": 1,
                      "cityName": "Bethlehem"
                    }
    ...
    @Operation(summary = "Get city by ID", description = "Returns a single city by its ID.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "City found", content = @Content(schema = @Schema(implementation = CityResponse.class), examples = @ExampleObject(name = "CityResponse", value = """
                    {
                      "id": 1,
                      "cityName": "Bethlehem"
                    }
    ...
    public ResponseEntity<CityResponse> getCityById(
            @Parameter(description = "City ID (must be positive)", example = "1", schema = @Schema(minimum = "1")) @PathVariable @Min(value = 1, message = "City ID must be greater than 0") Long id) {
        return ResponseEntity.ok(cityService.getCityById(id));
    }

    @GetMapping
    @PreAuthorize("hasAnyRole('ADMIN','RECEPTIONIST','ACCOUNTANT') and hasAuthority('CITY_READ')")
    @Operation(summary = "Get all cities", description = "Returns a list of all cities in the lookup table.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Cities returned successfully", content = @Content(schema = @Schema(implementation = CityResponse.class), examples = @ExampleObject(name = "CityListExample", value = """
                    [
    ...
    @Operation(summary = "Update a city", description = "Updates the city name for the given city ID.", requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "City update payload", required = true, content = @Content(examples = @ExampleObject(name = "UpdateCityRequest", value = """
            {
              "cityName": "Beit Sahour"
            }
            """))))
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "City updated successfully", content = @Content(schema = @Schema(implementation = CityResponse.class), examples = @ExampleObject(name = "UpdatedCity", value = """
                    {
                      "id": 1,
                      "cityName": "Beit Sahour"
                    }
    ...
    @Operation(summary = "Delete a city", description = "Deletes a city by ID. May fail if the city is referenced by customers.")
    @ApiResponses({
